...
export function NewTemplateDialog({ open, onOpenChange }: NewTemplateDialogProps) {
  const { addTemplate, users } = useApp();
    ...
    <Dialog open={open} onOpenChange={(isOpen) => { onOpenChange(isOpen); if(!isOpen) form.reset(); }}>
      <DialogContent className="sm:max-w-[625px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Konfigurieren Sie eine neue Vorlage für wiederkehrende Aufgaben.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter>
              <Button type="button" variant="ghost" onClick={() => { onOpenChange(false); form.reset(); }}>Abbrechen</Button>
